...
class PositionWrapper(gym.Wrapper):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[Any, Any]]:
        ...
        # Creating cubic spline interpolation points
        t = np.linspace(0, 1, self.NUM_ACTIONS_PER_STEP)
        cs = CubicSpline([0, 1], np.vstack([np.zeros(action.shape), action]), axis=0)
        ...
        for i in range(self.NUM_ACTIONS_PER_STEP):
            sub_action = cs(t[i])
            state, reward, terminated, truncated, info = self._take_single_step(sub_action)
